...
// add the following to your CMakeLists.txt file (or similar) to have
// all documentation code and strings excluded from your project
//			-DAS_GENERATE_DOCUMENTATION=0
...
int CScriptMgr::Init()
{
	...
	docgenOptions.includeArrayInterface = false;		// will clutter the example
	docgenOptions.includeStringInterface = false;		// will clutter the example
	docgenOptions.includeWeakRefInterface = false;		// will clutter the example
	...
	r = docGen.DocumentObjectType(engine->RegisterObjectType("CGameObj", 0, asOBJ_REF), "Represents a game object."); assert( r >= 0 );
	...
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameObj", "int get_x() const", asMETHOD(CGameObj, GetX), asCALL_THISCALL), "Returns current horizontal position for object in-game"); assert( r >= 0 );
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameObj", "int get_y() const", asMETHOD(CGameObj, GetY), asCALL_THISCALL), "Returns current vertical position for object in-game"); assert( r >= 0 );
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameObj", "bool Move(int dx, int dy)", asMETHOD(CGameObj, Move), asCALL_THISCALL), "Will attempt to move the object by the given amount, returns whether or not the move was legal."); assert( r >= 0 );

	// The script can kill the owning object
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameObj", "void Kill()", asMETHOD(CGameObj, Kill), asCALL_THISCALL), "Tell the game manager to kill the object."); assert( r >= 0 );
	...
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameObj", "void Send(ref msg, const CGameObj @+ to)", asMETHOD(CGameObj, Send), asCALL_THISCALL), "Send a message to another game object."); assert( r >= 0 );
	...
	r = docGen.DocumentObjectType(engine->RegisterInterface("IController"), "This class is used by the script manager to figure out which game classes are controllers."); assert( r >= 0 );



	// Register the game manager as a singleton. The script will access it through the global property
	r = docGen.DocumentObjectType(engine->RegisterObjectType("CGameMgr", 0, asOBJ_REF | asOBJ_NOHANDLE), "This class represents the global singleton game manager. There is a single instance available for it through the global <b>game</b> property."); assert( r >= 0 );
	...
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameMgr", "bool get_actionState(int idx)", asMETHOD(CGameMgr, GetActionState), asCALL_THISCALL), "Returns the current action state. The <i>idx</i> parameter can be 0 - 3 to fetch the state for the up, down, left or right keys. Returns if the key is currently pressed or not."); assert( r >= 0 );

	// The script can call this method to end the game
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameMgr", "void EndGame(bool win)", asMETHOD(CGameMgr, EndGame), asCALL_THISCALL), "This call ends the game. Indicate with the <i>win</i> parameter if the player won or not."); assert( r >= 0 );
	...
	r = docGen.DocumentObjectMethod(engine->RegisterObjectMethod("CGameMgr", "const CGameObj @+ FindObjByName(const string &in name)", asMETHOD(CGameMgr, FindGameObjByName), asCALL_THISCALL), "This function returns a reference to another game object. The <i>name</i> parameter must be set to a valid object name. If the object could not be found, null is returned."); assert( r >= 0 );
